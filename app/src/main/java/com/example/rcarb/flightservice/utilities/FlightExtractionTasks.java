...
public class FlightExtractionTasks {
    ...
    public static void executeTask(String action,
        ...
        if (ACTION_EXTRACT_SINGLE_FLIGHT_INFORMATION.equals(action)) {
            extractSingleFlight(context, id, flightName);
            ...
            getFlightsExcludedLandedCancelled(context);
            ...
            extratSingleFlightSecondAlarm(context, id, flightName);
...
//                context.sendBroadcast(sendToast);
    ...
    private static void getFlightsExcludedLandedCancelled(Context context) {
...
//                timeToParse = flightActualTime;
    ...
    private static void extractSingleFlight(Context context, long id, String flight) {
        ...
        FlightObject flightUpdated = ExtractFlightUtilities.getInfoForSIngleFlight(parseString);
        flightUpdated.setFlightName(flight);
        flightUpdated.setColumnId(id);

        String status = flightUpdated.getFlightStatus();

        //Check if alarm needs to be set.
        if (!flightUpdated.getFlightStatus().equals("Cancelled") ||
                !flightUpdated.getFlightStatus().equals("Landed")) {
            String a = "";
            //setup second alarm.
            Intent intent = new Intent(IntentActions.ACTION_SETUP_SECOND_ALARM);
            intent.putExtra(IntentActions.INTENT_SEND_PARCEL_FOR_SECOND_ALARM, flightUpdated);
            context.sendBroadcast(intent);
        }
    ...
    //Second alarm update
    private static void extratSingleFlightSecondAlarm(Context context, long id, String flight) {
        String site = "https://www.airport-la.com/lax/flight?flight_arrival=" + flight;
        String parseString = ExtractFlightUtilities.getFlightInfoFromWeb(site);
        FlightObject flightUpdated = ExtractFlightUtilities.getInfoForSIngleFlight(parseString);
        flightUpdated.setFlightName(flight);
        ...
        if (!flightUpdated.getFlightStatus().equals("Cancelled") ||
            ...
            //setup Third alarm.
            Intent intent = new Intent(IntentActions.INTENT_SEND_TOAST);
            intent.putExtra(IntentActions.INTENT_SEND_TOAST_MESSAGE, "Second Alarm failed");
            intent.putExtra(IntentActions.INTENT_SEND_TOAST_SPECIAL, "Second Alarm did not close status");
            context.sendBroadcast(intent);
        }
        //Update the database with the new information
        Intent intent = new Intent();
